...
) : IScopedService
{
	...
	private PreviewUser Render(User user, Dictionary<string, List<Emoji>> emoji)
	{
		...
		var res = new PreviewUser
		{
			...
			DisplayName    = mfm.RenderSimple(user.DisplayName, user.Host, mentions, emoji[user.Id], "span", true),
			Bio            = mfm.RenderSimple(user.UserProfile?.Description, user.Host, mentions, emoji[user.Id], "span", true),
