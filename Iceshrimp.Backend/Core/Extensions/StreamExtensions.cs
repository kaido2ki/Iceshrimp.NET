...
public static class StreamExtensions
{
	extension(Stream source)
	{
		public async Task CopyToAsync(
			Stream destination, long? maxLength, CancellationToken cancellationToken
		)
		{
		...
		public async Task<Stream> GetSafeStreamOrNullAsync(
			long? maxLength, long? contentLength, CancellationToken token = default
		)
		{
			...
			await source.CopyToAsync(buf, maxLength, token);
